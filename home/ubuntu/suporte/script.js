document.addEventListener("DOMContentLoaded", () => {
    ...
    function analisarProblema(textoProblema) {
        ...
        textoProblema = textoProblema.toLowerCase().trim();    // --- Funções Principais ---

    async function analisarProblema(textoProblema) { // Tornar a função async
        console.log("analisarProblema called with:", textoProblema);
        if (!resultadoAnalise) {
            console.error("resultadoAnalise is null inside analisarProblema!");
            return;
        }
        resultadoAnalise.innerHTML = "<p>Analisando com base de conhecimento interna e IA...</p>";

        // 1. Análise com Base de Conhecimento Interna (como antes)
        const palavrasChave = textoProblema.toLowerCase().match(/\b(\w+)\b/g) || [];
        ...
        baseConhecimento.problemas.forEach(problema => {
            ...
            problema.keywords.forEach(kw => {
                if (textoProblema.toLowerCase().includes(kw)) {
        ...
        let htmlResultadoInterno = "";
        if (melhorMatch && maxKeywordsMatch > 0) {
            htmlResultadoInterno = `<h4>Resultado da Base Interna:</h4>
                                    <p><strong>Setor Provável:</strong> ${melhorMatch.setor}</p>
                                    <p><strong>Possíveis Causas:</strong></p><ul>`;
            melhorMatch.causas.forEach(causa => htmlResultadoInterno += `<li>${causa}</li>`);
            htmlResultadoInterno += "</ul><p><strong>Sugestões / O que verificar:</strong></p><ul>";
            melhorMatch.solucoes.forEach(solucao => htmlResultadoInterno += `<li>${solucao}</li>`);
            htmlResultadoInterno += "</ul>";
            if (melhorMatch.contato && melhorMatch.contato.nome) {
                htmlResultadoInterno += `<p><strong>Contato:</strong> ${melhorMatch.contato.nome} (${melhorMatch.contato.funcao})</p>`;
            }
        } else {
            htmlResultadoInterno = `<p><strong>Base Interna:</strong> Nenhuma correspondência direta encontrada.</p>`;
        }

        // 2. Análise com IA Externa (via Flask Proxy)
        let htmlResultadoIA = "<h4>Análise Adicional com IA:</h4>";
        try {
            // Assume que o proxy Flask está rodando localmente na porta 5000
            const response = await fetch("https://suporte-producao-cg.netlify.app/analyze", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({ text: textoProblema }),
            });
            ...
            htmlResultadoIA += `<p><strong>Sentimento:</strong> Score ${aiData.sentiment_score?.toFixed(2)} (Magnitude: ${aiData.sentiment_magnitude?.toFixed(2)})</p>`;
            if (aiData.entities && aiData.entities.length > 0) {
                htmlResultadoIA += "<p><strong>Entidades Identificadas:</strong></p><ul>";
                aiData.entities.slice(0, 5).forEach(entity => { // Limita a 5 entidades para brevidade
                    htmlResultadoIA += `<li>${entity.name} (${entity.type}, Saliência: ${entity.salience?.toFixed(2)})</li>`;
                });
                htmlResultadoIA += "</ul>";
            } else {
                htmlResultadoIA += "<p>Nenhuma entidade principal identificada pela IA.</p>";
            }

        } catch (error) {
            console.error("Erro ao chamar a API de IA:", error);
            htmlResultadoIA += `<p style=\"color: red;\">Erro ao consultar a IA: ${error.message}</p>`;
        }

        // 3. Combina e exibe os resultados
        resultadoAnalise.innerHTML = htmlResultadoInterno + "<hr>" + htmlResultadoIA;
        console.log("Resultados combinados exibidos.");
        ...
        if (resposta) {
            ...
            if(respostasParciais.length > 0){
                 const htmlParcial = respostasParciais.join("");
                 resultadoBusca.innerHTML = htmlParcial;
            } else {
                 const htmlNotFound = `<p>Nenhuma informação rápida encontrada para \"${termoBusca}\". Tente palavras-chave diferentes ou consulte a análise de problemas, FAQ ou Workflows.</p>`;
                 resultadoBusca.innerHTML = htmlNotFound;
            }
        }
    }
    ...
    btnAnalisar.addEventListener("click", () => {
        ...
        // resultadoAnalise.innerHTML = "ANALYSIS DIV OK"; // Simple test assignment
        if (resultadoAnalise) {
            resultadoAnalise.style.backgroundColor = "yellow"; // Change background for visual test
            console.log("Changed background color of resultadoAnalise");
        } else {
            console.error("resultadoAnalise element is null!");
        }
